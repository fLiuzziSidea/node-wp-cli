var exec = require('child_process').exec, util = require('util'), escapeshellarg = require('escapeshellarg'), shellEscape = require('shell-escape');
...
module.exports.wp = function (use, args, flags, cb) {
  ...
  ['shell', 'uid', 'gid'].forEach(function(option) {
    if (flags[option]) {
      execOptions[option] = flags[option];
      delete flags[option];
    }
  });
  ...
  exec("wp " + shellEscape(args) + " " + flagArgs, execOptions, function (err, stdout, stderr) {
